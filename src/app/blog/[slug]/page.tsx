...
export default async function PostPage({
  ...
  const list = await fetchApi(`post`);
  const { posts }: { posts: PostPreviewType[] } = list;
  const morePosts = posts.filter((p) => p.slug !== postSlug).slice(0, 2);
      ...
      <section className="relative pt-12 pb-20 px-4 md:px-8 max-w-[1440px] mx-auto">
        <div className="max-w-[1240px] mx-auto space-y-8">
          ...
          <div className="flex items-center gap-2">
            <Tag
              variant="solid"
              className="px-4 py-2 uppercase tracking-widest text-xs"
            >
              {tags?.[0] || "BLOG"}
            </Tag>
          </div>
            ...
            {tags && tags.length > 0 && (
              <div className="mt-16 pt-8 border-t border-white/10 flex flex-wrap gap-3">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-white/40 hover:text-brand hover:border-brand/40 transition-colors"
                  >
                    #{tag}
                  </span>
                ))}
      ...
      <section className="py-24 px-4 md:px-8 max-w-[1440px] mx-auto">
        <div className="max-w-[1240px] mx-auto">
          <div className="flex items-center gap-4 mb-12">
            ...
            <h2 className="text-2xl font-archivo font-medium text-brand">
              More posts
            </h2>
          </div>
